...
- 2 Scale up能力
	...
	基于Redis的cluster目前采用了两种设计模式：proxy based和无中心的smart client based(如官方的Redis 3.0)。官方实现的Redis Cluster把系统设计成了一个"All-In-Box"模式的cluster，看似简洁易用轻便很多，但是谁用谁知道。目前业界没有简单比较成功的使用案例。

	Proxy-based Redis Cluster看似让架构复杂很多，但是这种架构很容易做到让每个模块自身职责明了清晰，升级也就方便很多，契合微服务的精神。目前比较成功的proxy有twemproxy，比较方便使用的Redis Cluster有Codis等。
...
如果client读到的value与写入的value一致，便可以认为写成功了，否则认为是失败的，不管中间的原因是Rm与Rs的同步太慢还是中间整个流程耗时过长导致了请求超时。如果某个Redis的写操作被认为是幂等的，则用户可以通过重试上面的流程来提高写成功率。这个方案需要redis和proxy配合一起保证数据的强一致性。
...
- I 定时检测超出quota的db，一旦其db容量下降到何时额度，就通知proxy开放对其进行写请求的访问；
- J 接收用户扩展其某个db的size指令，检验请求合法性并通过后，做出如下流程：

> 1 向各个proxy发出对某个db的写请求访问封禁通知；
>
> 2 收到所有的proxy响应后，从备用资源池抽出何时的instance上的db，作为migrate命令的参数发给keeper；
>
